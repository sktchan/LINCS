...
current_loss = Flux.logitcrossentropy(logits_masked, Flux.onehotbatch(y_masked, 1:n_classes))
...
return current_loss, logits_masked, y_masked
...
current_train_acc = total_train_masked > 0 ? total_train_tp / total_train_masked : 0.0

set_description(progress, "epoch $epoch [train] | loss: $(round(mean(epoch_train_losses), digits=4)) | acc: $(round(current_train_acc, digits=4))")
...
save_dir = joinpath("plots", "untrt", "indef_2", "$(timestamp)_epoch_$(epoch)")
