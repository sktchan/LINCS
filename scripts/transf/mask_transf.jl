...
#     # Define how many unique values to mask (from 1:978)
#     all_values = unique(X)
...
#     # Randomly pick values to mask (same across all columns)
...
open(params_txt, "w") do io
    ...
    println(io, "ADDITIONAL NOTES: trying 1 pos masked but diff across epochs/samples/test, model scaled up")
end

# println("smaug finished: trying 1 pos masked but diff across epochs/samples/test.\nyay!")

# results = Profile.Allocs.fetch()
# open("allocs_profile.txt", "w") do f
#     Profile.Allocs.view(f, results)
# end
# println("Allocation profile saved to allocs_profile.txt")
# jldsave(joinpath(save_dir, "allocs_profile.jld2"); results)
# println("Raw allocation profile data saved to $(joinpath(save_dir, "allocs_profile.jld2"))")
