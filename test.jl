...
result_dir = "results/predict_lincs_from_lincs/gene_prediction/test/ref_cl_MCF7_time_24h_dose_10uM"
if !isdir(result_dir)
    mkpath(result_dir)
end
...
# get landmark gene embeddings:
frogs_path = "../FRoGS"
frogs_gene_embedding_file = frogs_path * "/gene_vec_go_256.csv"
frogs_gene_to_id_file = frogs_path * "/term2gene_id.csv"
gene_to_gene_embedding_dict = create_gene_to_gene_embedding_dict(frogs_gene_to_id_file, frogs_gene_embedding_file)
...
target_cell_lines = get_target_cell_lines(lm_data, ref_criteria, target_criteria)
...
config["include_compound_embeddings"] && push!(include_vec, :compound_embeddings)  # Push if include flag == true
config["include_delta_ref"] && push!(include_vec, :delta_ref_profiles)
config["include_target_neutral_profile"] && push!(include_vec, :target_neutral_profile)
config["include_target_gene_embeddings"] && push!(include_vec, :target_gene_embeddings)
...
for target_cl in target_cell_lines
    ...
    if !isfile(data_file)
        ...
        data = create_data_struct(lm_data, 
    ...
    ## if data file exists, load it:
        ...
        data = load_data_from_hdf5(data_file)
    end
    ...
    if length(data.compounds) > 1  # We need at least two compounds to split the data
        ...
        if !isfile(splitted_data_file)
            splitted_data = SplittedData(data, include_vec)
...
splitted_data = load_splitted_data_from_hdf5(splitted_data_file)
